 ...
 */
function normalizePlayabilityError(status) {
  if (!status) {
    return {
      available: false,
  ...
  const reasonText =
    ...
    status.errorScreen?.playerErrorMessageRenderer?.subreason?.simpleText ||
    s;
...
function parsePlayer(raw) {
  ...
  if (playability && playability.status && playability.status !== 'OK') {
    const err = normalizePlayabilityError(playability);
    return {
      ...err,
