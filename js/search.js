...
function albumCard(a) {
  return `
    <a href="/album?id=${a.id || a.browseId}" class="album-card">
...
function playlistCard(p) {
  return `
    <a href="/playlist?id=${p.id || p.browseId}" class="album-card">
      <div class="art">${lazyImg(p.artwork, p.title)}</div>
...
async function doSearch(q) {
  ...
  try {
    ...
    if (data.albums?.length) {
      html += `<section class="result-section fade-in"><h2>Albums</h2><div class="album-grid">${data.albums.slice(0, 12).map(albumCard).join('')}</div></section>`;
    }
    if (data.playlists?.length) {
      html += `<section class="result-section fade-in"><h2>Playlists</h2><div class="album-grid">${data.playlists.slice(0, 12).map(playlistCard).join('')}</div></section>`;
    }
