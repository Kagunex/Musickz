...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!data.available) {
      ...
      // Do NOT force Sign In for bot/client failures
      const message =
        data.reason ||
        (code === 'LOGIN_REQUIRED'
      ...
      // 403 only for true account auth requirement
      const status =
        code === 'LOGIN_REQUIRED' ? 403 : code === 'INVALID_ID' ? 400 : 502;

      console.log(
        `[player] videoId=${id} unavailable code=${code} category=${data.playabilityCategory || '-'} client=${data.client || '?'} tried=${(data.clientsTried || []).join(',')}`
      );
      return res.status(status).json(
        error(code, message, {
          playabilityStatus: data.playabilityStatus || null,
          playabilityCategory: data.playabilityCategory || null,
          hasCipher: !!data.hasCipher,
          clientsTried: data.clientsTried || null,
        })
      );
    }
