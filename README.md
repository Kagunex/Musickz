...
## Playback resolver (2026)

MusicKx playback uses `youtubei.js` with a multi-client resolver. The resolver tries `VISIONOS` first, then `WEB_REMIX`, `YTMUSIC`, and `IOS`, stopping at the first client that returns a valid audio format and a deciphered HTTPS stream URL.

This mirrors the current ecosystem's client-fallback approach rather than assuming one InnerTube client will always work. YouTube can change client availability, bot checks, stream formats, and regional/content restrictions at any time, so no anonymous resolver can honestly guarantee every track will always be playable.

The resolver never creates fake stream URLs and does not use account cookies or attempt to bypass actual account, age, region, or private-content restrictions.
